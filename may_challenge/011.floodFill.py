...
class Solution:
    def floodFill(self, image: List[List[int]], sr: int, sc: int, newColor: int) -> List[List[int]]:
        return self.findNeighbors(image, sr, sc, image[sr][sc], newColor)

    def findNeighbors(self, image, r, c, oldColor, newColor):
        image[r][c] = newColor

        if (oldColor != newColor):
            if (r-1 > -1 and image[r-1][c] == oldColor):
                image = self.findNeighbors(image, r-1, c, oldColor, newColor)

            if (r+1 < len(image) and image[r+1][c] == oldColor):
                image = self.findNeighbors(image, r+1, c, oldColor, newColor)

            if (c-1 > -1 and image[r][c-1] == oldColor):
                image = self.findNeighbors(image, r, c-1, oldColor, newColor)
